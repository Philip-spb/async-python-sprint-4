...
### Для запуска сервера без докера

1. Создать базу в докере `docker-compose up -d postgresql`
2. Установить виртуальное окружение и не забыть его активировать
3. Установить все зависимости из `requirements.txt`
4. Из файла `.env.example` создать файл `.env`
5. Выполнить миграции `alembic -c src/alembic.ini upgrade head`
    - Посмотреть текущее состояние миграций `alembic -c src/alembic.ini current`
    - Создать новую миграцию `alembic -c src/alembic.ini revision --autogenerate -m 01_initial-db`
    - Откатить одну последнюю миграцию `alembic -c src/alembic.ini downgrade -1`
6. Выполнить проверку на соответствие кода проекта стандарту PEP8 `flake8`
7. Выполнить тесты `pytest`
8. Запустить проект `python src/main.py`
    - Также проект можно запустить командой `uvicorn --app-dir src main:app`

### Для запуска сервера в докере

1. Из файла `.env.docker.example` создать файл `.env.docker`
